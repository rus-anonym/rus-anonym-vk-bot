...
new UserCommand({
		regexp: /(?:^!инфо|!info)(?:\s(.*))?$/i, process: async function (message) {
			...
			return message.reply({
				...
				message: `@id${userData.id} (${userData.info.name} ${userData.info.surname}):
Статус: ${userData.info.extends.status}
Сообщений: ${userData.messages.length}
Сообщений в ЛС: ${userData.personalMessages.length}
Сообщений в беседах: ${userData.messages.length - userData.personalMessages.length}
Зарегистрирован в ВК: ${moment(
					await utils.vk.user.getUserRegDate(userData.id)).format("DD.MM.YYYY, HH:mm:ss")}
Последнее изменение данных в ВК: ${moment(
						await utils.vk.user.getUserModifiedDate(userData.id)).format("DD.MM.YYYY, HH:mm:ss")}
...
Текущий статус: ${userData.info.last_seen.isOnline ? `Онлайн` : `Офлайн`}\n`
						: ""}Упоминания пользователя: https://vk.com/feed?obj=${userData.id}&q=&section=mentions`,
			});
		}
	});
