...
new UserCommand({
	...
	process: async function (message) {
		...
		try {
			const [userStatus] = await VK.master.getAPI().friends.areFriends({
				user_ids: userID,
			});
			...
			if (message.state.args[1] === "+") {
				...
				const response = await VK.master.getAPI().friends.add({
					user_id: userID,
				});
				return message.reply({
					disable_mentions: true,
				...
				const response = await VK.master.getAPI().friends.delete({
					user_id: userID,
				});
				let text = ``;
