...
function createUserUnblockMiddleware(
	subGroup: SubGroupVK,
): (event: GroupUserContext) => Promise<void> {
	return async function userUnblock(event: GroupUserContext): Promise<void> {
		if (event.isExpired) {
...
export default createUserUnblockMiddleware;
